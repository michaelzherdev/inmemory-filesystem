...
import org.scala.oop.files.{Directory, File}
...
class Echo(args: Array[String]) extends Command {
  ...
  def getRootAfterEcho(currentDirectory: Directory, path: List[String], contents: String, append: Boolean): Directory = {
    if (path.isEmpty) currentDirectory
      ...
      val newNextDir = getRootAfterEcho(nextDirectory, path.tail, contents, append)

      if (nextDirectory == newNextDir) currentDirectory
  ...
  def doEcho(state: State, contents: String, filename: String, append: Boolean): State = {
    if (filename.contains(Directory.SEPARATOR))
      state.setMessage("Echo: filename must mot contain separator!")
    else {
      val newRoot: Directory = getRootAfterEcho(
        state.root, state.workDirectory.getAllFoldersInPath :+ filename, contents, append)
      if (newRoot == state.root)
        state.setMessage(filename + ": no such file")
      else
        State(newRoot, newRoot.findDescendant(state.workDirectory.getAllFoldersInPath))
    }
  }
}
